...
SEXP est3( SEXP model, SEXP delta, SEXP gamma, 
  ...
  initParmHealth3( region, theta, health_parm, ptr_res_seg, ptr_m,
...
extern "C" SEXP est2( SEXP model, SEXP delta, SEXP gamma, 
                     SEXP alpha, SEXP beta, SEXP lambda2, 
                     SEXP m, SEXP nu2, SEXP maxit );
SEXP est2( SEXP model, SEXP delta, SEXP gamma, 
           SEXP alpha, SEXP beta, SEXP lambda2, 
           SEXP m, SEXP nu2, SEXP maxit ) {
  SEXP info = getListElement( model, "info" );
  SEXP seg = getListElement( model, "seg" );
  ...
  // a copy of seg and beta
  SEXP res_seg = PROTECT( allocMatrix( INTSXP, 2, len ) );
  SEXP res_beta = PROTECT( allocVector( REALSXP, 2 ) );
  int *ptr_res_seg = INTEGER( res_seg );
  for( int i = 0; i < 2 * len; ++ i ) {
    ptr_res_seg[ i ] = old_seg[ i ];
  }
  ...
  // 0, 1= -1, -2
  vector<double> health_parm( 8 * 2, 0 );
  ...
  // Initialize parameters for healthy regions
  initParmHealth2( region, theta, health_parm, ptr_res_seg, ptr_m,
                   ptr_nu2, ptr_intst, ptr_lambda2, ptr_nidx,
                   ptr_nintst, ptr_alpha, 
                   ptr_res_beta, len, 20 );
  updateBeta2( ptr_res_beta, ptr_alpha, health_parm );
  
  for( int i = 0; i < *ptr_maxit; ++ i ) {
    for( int j = 1; j <= len; ++ j ) {
      cmpE2( j, health_parm, ptr_res_seg,ptr_nidx, ptr_intst, ptr_nintst, 
             ptr_delta, ptr_gamma, theta );
  ...
  double *ptr_res_parm = REAL( res_parm );
  int *ptr_res_image = INTEGER( res_image );
  copyParmHealth2( health_parm, ptr_res_parm, nrow );
  restoreImg( ptr_idx, ptr_res_seg, ptr_res_image, len );
